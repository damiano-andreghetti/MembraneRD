...
function run_RD(cfg_memb::Matrix{Int}, cfg_cyto::Vector{Int}, neig::Matrix{Int}, centers::Matrix{Float64}, T::Float64, Tmeas::Float64, dA::Float64, dB::Float64,dEA::Float64,dEB::Float64,kAc::Float64, kBc::Float64, kAa::Float64,kAd::Float64,kBa::Float64,kBd::Float64, KMM::Float64,rho_0::Float64, seed::Int,Nsave::Int,fold_files::String)
	...
	while t <T
		...
		while last_meas<t+dt
			...
			last_meas+=Tmeas
			cnt_save+=1
			#the following is to check proportionality between EA and A
			#perm=randperm(Nc)
			#Avals=block_ave(cfg_memb[perm,1],Int(Nc/4))
			#EAvals=block_ave(cfg_memb[perm,3]*kAd/(kAa*cfg_cyto[1]),Int(Nc/4))
			#display(scatter!(Avals,EAvals,xlim=(0,6),ylim=(0,6)))
			#the following is to check for convergence of rho to 1
			rho=rho_0*((kAd/kAa)+totA)/((kBd/kBa)+totB)
			measures[cnt_meas,5]=rho
			#display(plot([scatter(collect(1:cnt_meas).*Tmeas,measures[1:cnt_meas,5]),scatter(centers[:,1],centers[:,2],clim=(-1,1),zcolor=(cfg_memb[:,2].-cfg_memb[:,1])./(cfg_memb[:,1].+cfg_memb[:,2]),color=:vik,title="T="*string(round(last_meas,digits=3)),markerstrokewidths=0.0,markershape=:hexagon,markersize=0.8,label=L"\varphi",aspect_ratio=:equal)]...,layout=(2,1)))
			cnt_meas+=1
			#this saves measures
